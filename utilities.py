...
def orderstr(order):
    ...
    try:
        s = 'Market response: '
        s += 'Id:' + str(order['orderId']) + ' status:' + str(order['status']) + ' side:' + str(
            order['side']) + ' type:' + str(order['type']) + ' quantity:' + str(order['origQty'])
        if order['type'] == 'LIMIT':
...
def timestr(dateTime: int, end='f'):
    """
    Returns string representation for an integer time
    """
    if end == 'm':
        s = pd.to_datetime(dateTime, unit='ms').strftime("%y-%m-%d %H:%M")
    elif end == 's':
        s = pd.to_datetime(dateTime, unit='ms').strftime("%y-%m-%d %H:%M:%S")
    elif end == 'f':
        s = pd.to_datetime(dateTime, unit='ms').strftime("%y-%m-%d %H:%M:%S:%f")[:-3]
    return s
